...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Find user by phone (or admin ID passed as phone)
        ...
        if (type === 'Potholes') {
          ...
          if (!customModel) {
            ...
            try {
              ...
              // If potholes probability (index 0) is less than 50%
              if (predictionData[0] < 0.5) {
                return res.status(400).json({
                  message: `The uploaded image does not appear to contain a pothole. Please upload a valid photo.`
                });
              }
            } catch (mlError) {
          ...
          if (!net) {
            ...
            try {
              ...
              if (!isValid) {
                return res.status(400).json({
                  message: `The uploaded image does not appear to match the selected category '${type}'. Please upload a relevant photo.`
                });
              }
            } catch (mlError) {
